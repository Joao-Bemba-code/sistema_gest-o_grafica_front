          ...
          <div className="w-full max-w-md border-gradient rounded-3xl bg-card shadow-2xl p-6 animate-pop max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                <p className="text-[11px] font-black uppercase tracking-wider text-muted-foreground">Onde estão os dados</p>
              ...
              <button
                ...
                onClick={guardar}
                disabled={guardando}
                className="w-full h-10 bg-brand hover:brightness-110 text-white text-xs font-bold rounded-xl shadow-md shadow-primary/25 transition-all active:scale-[0.98] disabled:opacity-50"
              >
                {guardando ? "A guardar..." : "Guardar e reiniciar"}
              </button>
              <p className="text-[10px] text-muted-foreground text-center">
                A aplicação reinicia para aplicar a alteração.
              </p>
              <p className="text-[10px] text-muted-foreground text-center">
                A sincronização com o site configura-se nas <span className="font-semibold">Configurações</span> após entrar.
              </p>
            </div>
          </div>
        </div>
